...
public class ExcelUtility
{
  ...
  public void ConvertToJson(string JsonPath, Encoding encoding)
  {
    ...
    for (int i = 1; i < rowCount; i++)
    {
      ...
      string key = mSheet.Rows[i][0].ToString();
      //添加到表数据中
      table.Add(key, row);
    }
  ...
  public void ConvertToXml(string XmlFile)
  {
	...
	/// 转换为language txt 资源
	/// </summary>
	public void ConvertToTXT(string TXTPath, Encoding encoding,LanguageConfig cfg)
	{
		//判断Excel文件中是否存在数据表
		if (mResultSet.Tables.Count < 1)
			return;

		for (int count = 0; count < mResultSet.Tables.Count; count++) {
			//默认读取第一个数据表
			DataTable mSheet = mResultSet.Tables[count];
			...
			//创建一个StringBuilder存储数据
			Dictionary<string,StringBuilder> builderList = new Dictionary<string,StringBuilder>();

			DataRow keyRow = mSheet.Rows [0];
			//读取数据
			for (int i = 1; i < rowCount; i++)
			{
				for (int j = 1; j < colCount; j++)
				{
					...
					string tempDicKey = keyRow [j].ToString();
					if (builderList.ContainsKey (tempDicKey)) {
						builder = builderList [tempDicKey];
					} else {
						builder = new StringBuilder ();
						builderList.Add (tempDicKey, builder);
					}

					//使用","分割每一个数值
					string itemkey = cfg.GetKeys()[count];
					if (string.IsNullOrEmpty(itemkey)) {
						string key = cfg.GetModule() +"."+mSheet.Rows[i][0];
						builder.Append(key + "=" +mSheet.Rows[i][j] );
					} else {
						string key = cfg.GetModule() + "."+ itemkey + "."+ mSheet.Rows[i][0];
						builder.Append(key + "=" +mSheet.Rows[i][j] );
					}
					builder.Append("\r\n");
				}
			...
			foreach (KeyValuePair<string, StringBuilder> pair in builderList)  
			{  
				//Debug.Log(pair.Key + " " + pair.Value);
				//写入文件
				string path = TXTPath + pair.Key.ToUpper() + ".txt";

				StringBuilder builder = pair.Value;
